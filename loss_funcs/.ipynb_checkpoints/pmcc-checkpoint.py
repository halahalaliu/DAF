...
class LNMLoss(MMDLoss, LambdaSheduler):
    ...
    def forward(self, source, target, source_label, target_logits):
        if self.kernel_type == 'linear':
            ...
            _, out, _ = torch.svd(target_logits)
            loss_bnm = -torch.mean(out)

            loss = loss_lmmd + loss_bnm
            return loss
